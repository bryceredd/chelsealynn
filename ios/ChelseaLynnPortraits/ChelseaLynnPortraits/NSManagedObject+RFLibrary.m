...
+ (RFItemBlock) mapJSON:(RFItemBlock)callback {
    return [^(id data) {
        id obj = [self objectWithObject:data inContext:[DDPersist mainContext] upsert:YES];
        [[DDPersist mainContext] persist];
...
+ (NSFetchedResultsController*) observeWithPredicate:(id)predicateOrString sort:(NSString*)key ascending:(BOOL)ascending callback:(RFArrayBlock)callback {
    ...
    NSManagedObjectContext* context = [DDPersist mainContext];
